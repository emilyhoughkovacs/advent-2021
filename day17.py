...
class Solution(object):
    ...
    def solution(self, args):
        ...
        points = [[7, 2], [6, 3], [9,0], [17, -4], [6, 9]]
        theAnswer = -999999999
        for a in range(targetArea[0][1]+1):
            for b in range(-1*targetArea[1][0]):
                pos = [0, 0]
                maxY = b
                ...
                while not pos_inside_target(pos) and pos[1] >= targetArea[1][0] and pos[0]<=targetArea[0][1]:
                    ...
                    if pos[1]> maxY:
                        maxY = pos[1]
                if pos_inside_target(pos) and maxY > theAnswer:
                    theAnswer = maxY
                    print(pos, x, y, theAnswer)
                print("===============")

        # not sure why this magic works but apparently it does
        # maxVal = sum([i for i in range(-1*targetArea[1][0])])
        # print(maxVal)
